...
#TSP base case
arr = [ [INF for _ in range(num_cities)] for _ in range(int(math.pow(2,num_cities-1)))]
arr[0][0] = 0  # Base case where S={1} and j=1 which is 0 in this case
            ...
            for j in range(1,num_cities):  # j!=city0
                ...
                for k in range(num_cities):
                    if k != j:
                        ...
                        # DEBUG print 'm=%d i=%d bini=%s j=%d k=%d arr[%d][%d]=%d dist[%d][%d]=%d' % (m,i>>1,bini,j,k,index,k,arr[index][k],k,j,dist[k][j])
                        if tmpk < mink:
                            mink = tmpk
                arr[i>>1][j] = mink
        gc.collect()
...
for j in range(num_cities-1):
    ind = int(math.pow(2,num_cities-1))
    ind -= 1
